...
locations = [["Hamburg Messehallen","https://001-iz.impfterminservice.de/impftermine/service?plz=76646", "https://353-iz.impfterminservice.de/rest/suche/termincheck?plz=20357&leistungsmerkmale=L920,L921,L922,L923"]]
...
async def scrapeAsyncPage(locationData, remote):
...
async def startAsyncScraping():
    while True:
        for server in servers:
            for location in locations:
                print("Creating...")
                asyncio.create_task(scrapePage(location, server))
...
while(True):
    ...
    #     time.sleep(30)
    asyncio.run(startAsyncScraping())